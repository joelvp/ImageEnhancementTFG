import logging
from models.sky_replace.skybox_utils import *
from cv2.ximgproc import guidedFilter

class SkyBox():

    def __init__(self, args, sky_image):

        self.args = args
        self.load_skybox(sky_image)
    ...
    def load_skybox(self, sky_image):

        logging.info('Initialize skybox...')
        
        image_ratio = self.args.out_size_w - self.args.out_size_h
        sky_image_ratio = sky_image.shape[1] - sky_image.shape[0]


        # if '.jpg' not in self.args.skybox: esta condicion es por si se puede meter video
        if True:
            # static backgroud
            skybox_img = sky_image
            
            if image_ratio < 0 and sky_image_ratio > 0: # Si la imagen es vertical y el fondo horizontal
                logging.info('Imagen vertical, fondo horizontal')
                reference_ratio = self.args.out_size_w / self.args.out_size_h
                
                crop_width = min(skybox_img.shape[1], int(skybox_img.shape[0] * reference_ratio))
                crop_height = min(skybox_img.shape[0], int(skybox_img.shape[1] / reference_ratio))
                
                # Calcular las coordenadas de recorte para centrar la imagen
                x = (skybox_img.shape[1] - crop_width) // 2
                y = (skybox_img.shape[0] - crop_height) // 2
                
                # Recortar la imagen
                skybox_img = skybox_img[y:y + crop_height, x:x + crop_width]
    ...
    def skybox_tracking(self, frame, frame_prev, skymask):
        ...
        if np.mean(skymask) < 0.05:
            logging.warning('sky area is too small')
        ...
        if prev_pts is None:
            logging.warning('no feature point detected')
            return np.array([[1, 0, 0], [0, 1, 0]], dtype=np.float32)
        ...
        if idx.size == 0:
            logging.warning('no good point matched')
            return np.array([[1, 0, 0], [0, 1, 0]], dtype=np.float32)
