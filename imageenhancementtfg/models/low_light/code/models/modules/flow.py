...
from models.low_light.code.models.modules.FlowActNorms import ActNorm2d
from . import thops
...
class GaussianDiag:
    ...
    def logp(mean, logs, x):
        ...
        if isinstance(x, (list, tuple)):
            for x_ in x: likelihood += thops.sum(GaussianDiag.likelihood(mean, logs, x_), dim=[1, 2, 3])
        else:
            likelihood = thops.sum(GaussianDiag.likelihood(mean, logs, x), dim=[1, 2, 3])
        return likelihood
