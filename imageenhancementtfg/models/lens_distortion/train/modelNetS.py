...
class EncoderNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn(self.conv(x))) # Tensor size [32, 64, 256, 256]

        x = self.en_layer1(x)     # Tensor size [32, 64, 256, 256]
        x = self.en_layer2(x)     # Tensor size [32, 128, 128, 128]
        x = self.en_layer3(x)     # Tensor size [32, 256, 64, 64]
        x = self.en_layer4(x)     # Tensor size [32, 512, 32, 32]
        x = self.en_layer5(x)     # Tensor size [32, 512, 16, 16]
        
        x = F.relu(self.bn1(self.conv1(x))) # Tensor size [32, 512, 8, 8]
        x = F.relu(self.bn2(self.conv2(x))) # Tensor size [32, 512, 4, 4]
        
        x = x.view(x.size(0), -1) # Tensor size [32, 8192]
        para = self.fc(x) # Tensor size [32, 1]
